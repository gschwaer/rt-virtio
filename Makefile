...
CPPFLAGS =	$(ARCH_CPPFLAGS) -MD -MF $@.d
LDFLAGS = $(GENERIC_FLAGS) $(ARCH_LDFLAGS) -nostartfiles 
LIBS:=
ifneq ($(JAILHOUSE_PATH),)
ifneq ($(ARCH), armv8)
$(error Makefile only supports armv8 platforms.)
endif
CPPFLAGS+=-DSINGLE_CORE=y -DVIRTUALIZED_GICC=y
INC_DIRS+=$(realpath $(addsuffix /include, $(JAILHOUSE_PATH))) \
	  $(realpath $(addsuffix /include/arch/arm64, $(JAILHOUSE_PATH))) \
	  $(realpath $(addsuffix /inmates/lib/include, $(JAILHOUSE_PATH))) \
	  $(realpath $(addsuffix /inmates/lib/arm64/include, $(JAILHOUSE_PATH))) \
	  $(realpath $(addsuffix /inmates/lib/arm-common/include, $(JAILHOUSE_PATH)))
LIBS+=$(realpath $(addsuffix /inmates/lib/arm64/lib.a, $(JAILHOUSE_PATH)))
endif
ifneq ($(USE_SECONDARY_UART),)
CPPFLAGS+=-DUSE_SECONDARY_UART=y
endif
...
$(TARGET).elf: $(OBJS) $(GEN_LD_FILE) $(LIBS)
	$(CC) $(LDFLAGS) -T$(GEN_LD_FILE) $(OBJS) $(LIBS) -o $@
